...
// **바닥 이하로는 못 가져간다** (밸런싱에서 넣었다 — `FARM.md` 6단계).
// 바닥은 하루 생산량 × `RAID_FLOOR_DAYS` — 지금 **이틀치**다. 「하루치는 남긴다」는
// 이 규칙의 별명이고 수치는 이 상수 하나가 정한다 (별명대로 1로 읽으면 안 된다).
// 이삭이 바닥 이하로 남아 있으면 이겨도 이삭은 한 개도 안 나온다.
