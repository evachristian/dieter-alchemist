...
//
//  ⚠️ **기본은 켜져 있고, `DEV_TOOLS=0` 으로 끈다.**
//
//  처음에는 반대로 만들었다(`DEV_TOOLS=1` 일 때만 켜짐). 「배포된 서버에서 누구나
//  제 밭을 채우면 안 된다」는 이유였는데, **이 저장소의 실제 태도와 어긋났다** —
//  같은 개발용 블록의 「모든 재료 1000개씩」·「모든 맵 오픈」·「AP 충전」은
//  전부 아무 잠금 없이 배포된다. 밭 채우기만 환경 변수를 요구할 이유가 없고,
//  그 때문에 버튼이 안 먹는다는 신고가 왔다.
//
//  출시할 때는 개발용 블록을 통째로 지운다(`.dev-tools`). 그 전에 서버만 먼저
//  잠그고 싶으면 `DEV_TOOLS=0` 하나로 끈다.
