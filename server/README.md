...
| `POST` | `/api/farm/:playerId/dev` | ⚠️ **개발용** — 밭을 상한까지 채운다. `DEV_TOOLS=1` 일 때만 살아 있고, 아니면 **404 `dev_off`** |
...
### ⚠️ 개발용 스위치 (`DEV_TOOLS`) — **기본은 꺼져 있다**

탐험 탭의 「개발용(임시)」에 **🌾 모든 밭 생산 완료** 버튼이 있다.
밭은 서버가 정본이라 (아래) 화면에서 흉내 낼 수가 없어서, 이것만은 서버에 있다.

```bash
DEV_TOOLS=1 npm start      # 켜고 띄우기 (로컬에서만)
```

**배포에는 절대 넣지 않는다.** 공개 저장소이고 도메인이 열려 있어서, 켜져 있으면
누구나 제 밭을 공짜로 채운다 — 그렇게 채운 것은 **남이 털어 갈 수도 있어서**
밭 경제가 통째로 무너진다. 꺼져 있을 때는 404 이고, 몸통이 `dev_off` 다
(세이브가 아직 서버에 없는 사람의 404 `not_found` 와 구분하려는 것이다 —
안 그러면 화면이 엉뚱한 사람에게 「DEV_TOOLS 를 켜세요」를 띄운다).
