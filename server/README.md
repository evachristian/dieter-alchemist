...
### 개발용 스위치 (`DEV_TOOLS`) — **기본은 켜져 있다**

탐험 탭의 「개발용(임시)」에 **🌾 모든 밭 생산 완료** 버튼이 있다.
밭은 서버가 정본이라 (아래) 화면에서 흉내 낼 수가 없어서, 이것만은 서버에 있다.

```bash
DEV_TOOLS=0 npm start      # 서버 쪽만 먼저 잠그고 싶을 때
```

⚠️ **처음에는 반대로 만들었다** (`DEV_TOOLS=1` 일 때만 켜짐). 「배포된 서버에서
누구나 제 밭을 채우면 안 된다」는 이유였는데, **이 저장소의 실제 태도와 어긋났다** —
같은 개발용 블록의 「모든 재료 1000개씩」·「모든 맵 오픈」·「AP 충전」은 전부
아무 잠금 없이 배포된다. 밭 채우기만 환경 변수를 요구할 이유가 없었고,
그 때문에 **버튼이 안 먹는다는 신고가 왔다.**

출시할 때는 **개발용 블록을 통째로 지운다**(`.dev-tools`). 그것이 원래 정해 둔
방법이고, `DEV_TOOLS=0` 은 그 전에 서버만 먼저 잠그고 싶을 때 쓰는 임시 손잡이다.
꺼져 있을 때는 404 이고 몸통이 `dev_off` 다 (세이브가 아직 서버에 없는 사람의
404 `not_found` 와 구분하려는 것이다 — 안 그러면 화면이 엉뚱한 사람에게
「개발용 도구가 꺼져 있어요」를 띄운다).
...
- **밭을 한 번도 안 연 사람은 상대 목록에 안 나온다.** 기능이 있는 줄도 모르는 채로
  털리는 일이 없어야 한다
- **털린 밭에는 네 시간 방패가 걸린다.** 이것이 없으면 자는 사이에 밭이 열 번 털린다.
  (밸런싱에서 2시간 → 4시간으로 늘렸다 — 2시간이면 하루 **12번**까지 털린다. `FARM.md` 9-1)
- **하루치는 못 가져간다** (`RAID_FLOOR_DAYS`). 이삭이 그 사람의 하루 생산량 × 2일 이하로
  남아 있으면 이겨도 이삭은 한 개도 안 나온다. 방패는 *연달아* 털리는 것만 막아서,
  며칠 못 들어온 사람은 결국 바닥까지 긁혔다. **다 자란 작물에는 안 걸린다**
- ⚠️ **남의 밭도 목록·약탈에서 제 시계로 자라게 한다.** 예전에는 주인이 들어와야만
  자랐는데 주인은 들어오면 곧바로 거둔다 — **저장된 밭이 늘 비어 있어 약탈이
  30일 동안 0번**이었다 (`node tools/simfarm.js` 로 잡았다). 남이 불러도 안전한 이유:
  `grow()` 는 시계를 `now - (elapsed % 24h)` 로 당겨 **24시간의 위상을 보존한다**
