...
(function () {
  ...
  function graduate() {
    ...
    // **바디파츠를 전부 상한(150%)에서 시작한다.** 이 게임은 날씬해지는 이야기라
    // 100% 로 출발하면 첫 물약부터 «표준보다 마른» 몸이 되어 줄어들 자리가 없다.
    // ⚠️ 세이브가 아니라 `bodyTune` 의 제 열쇠에 사는 값이라 마이그레이션이 아니라
    // «졸업할 때 한 번»이다 — 이미 졸업한 사람이 고른 값을 뺏지 않는다
    if (typeof maxTune === 'function') maxTune();
    // ⚠️ **여기서 첫 퀘스트를 내보낸다.** `refreshQuests()` 는 켤 때와 매력이 오를 때만
    // 도는데 튜토리얼을 막 마친 사람은 둘 다 아니라, **칩이 안 뜨고 할 일이 없었다**
    // (신고받은 자리다). `finish()` 가 아니라 **여기**인 이유는 `tutorialDone` 을
    // 켜는 곳이 여기라서다 — 졸업이 어느 길로 오든 퀘스트가 같이 따라온다
