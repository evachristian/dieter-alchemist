...
// ⚠️ **여왕의 이름이 화면에 새면 안 된다** (STORY.md 「여왕」 · 마지막 장면의 전부다).
// 브라우저가 필요 없는 절반만 여기서 돌린다 — `npm test` 마다 봐야 하는 약속이라서다.
// 화면까지 훑는 나머지 절반은 `npm run test:name` 이다. **식은 저기 한 곳에 있다**
require('./checkname.js').scanStrings().forEach(m => bad.push(m));
