...
//
// 보는 것 셋 —
//   ① **머리 껍질** : 제일 두꺼운 데 ÷ 제일 얇은 데 ≤ 1.5 · 두개골 위로 뜬 몫 ≤ 6px
...
const SHELL_MAX = 1.5;
const LIFT_MAX = 6;       // 머리카락 꼭대기가 두개골 꼭대기보다 몇 px 위까지 봐 주는가
...
(async () => {
  ...
  out.forEach(r => {
    ...
    if (r.lift > LIFT_MAX) bad.push(`${r.hair}: 머리카락이 두개골 위로 ${r.lift}px 떠 있다 (${LIFT_MAX} 까지)`);
  ...
  if (!out.length || angles === 0) bad.push('한 스타일도 못 쟀다 — 0건이 통과가 아니다');

  console.log(`\n잰것: 머리 모양 ${out.length}가지 × 각도 ${ANGLES.length} (${angles}번) · 틈은 ${rows}줄`);
