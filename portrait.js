...
(function () {
  ...
  // ─── 장식 ─────────────────────────────────────────────────
  // 왕관 띠의 반폭과, 그 띠의 «밑변»이 옛 그림에서 서 있던 y. 둘은 짝이다 —
  // 띠를 넓히거나 좁히면 앉는 높이가 같이 따라와야 머리에서 안 뜬다
  const CROWN_HALF = 20, CROWN_BOT = 28;
  const DECO = {
    none:    () => '',
    // ⚠️ **왕관도 «그 사람이 쓴 머리»에서 자리를 뽑는다** (후드·서클렛과 같은 규칙이다).
    // 오래 `y22` 에 박혀 있었는데, 머리 부피를 「옛것과 새것의 중간으로」 키우면서
    // 돔이 내려와 **띠가 머리 위에 떠 버렸다** — 이그리트의 x=40 에서 5px 이 비쳤다
    // (「머리와 얼굴 사이」 검사가 그것을 잡았다). 띠의 폭(`CROWN_HALF`)은 그대로 두고
    // **그 폭이 되는 높이를 돔에서 구해** 거기에 앉힌다: 머리를 두껍게 고치면
    // 왕관이 따라 올라가고, 얇게 고치면 따라 내려온다.
    // ⚠️ 가로로 늘리지 않는다 — 왕관의 «생김새»는 그대로이고 자리만 따라간다
    crown:   (c, hair) => {
      const h = HAIR_SPEC[hair] || { side: 5, top: 8 };
      const rx = SKULL.rx + h.side, ry = SKULL.ry + h.top;
      const k = Math.sqrt(Math.max(0, 1 - Math.min(1, CROWN_HALF / rx) ** 2));
      const dy = +(SKULL.cy - ry * k - CROWN_BOT).toFixed(2);   // 옛 자리에서 얼마나 옮기는가
      return `<g transform="translate(0,${dy})">`
        + `<path d="M40,22 L46,10 L52,20 L60,6 L68,20 L74,10 L80,22 Z" fill="${c}"/>`
        + `<rect x="40" y="22" width="40" height="6" rx="2" fill="${c}"/></g>`;
    },
