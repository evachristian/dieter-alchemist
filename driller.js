...
(function () {
  ...
  //
  // 아무 생각 없이 아래만 파는 손은 **50초에 산소가 바닥나** 그 절반쯤에서 멈춘다 —
  // 2분을 다 쓰려면 **에어 캡슐을 주우러 돌아야** 하고, 그것이 곧 이 게임의 실력이다
  // (미스터 드릴러의 축 그대로다).
  // ⚠️ **봇 속도가 깊이에 따라 흔들린다** — 얕은 데는 단단한 바위가 드물어 빠르고
  // 깊어질수록 느려진다. 그래서 **20초는 재야** 값이 모인다.
  //
  // ⚠️⚠️ **판을 꽉 채우면서 상한을 다시 잡았다** (`ensureRows` 의 주석).
  // 구멍이 있던 판에서는 봇이 **17~21 m/s** 였는데, 그중 3분의 1은 파지도 않고
  // 구멍으로 떨어진 몫이었다. 꽉 찬 판은 **한 칸도 안 파면 한 줄도 못 내려가서**
  // 같은 봇이 **10.7~12.2 m/s**(다섯 판)로 내려왔다 — 그래서 1,800m 는 2분을
  // 다 써도 못 닿는 «장식»이 됐다. 지금은 **990m**(55m 에 하나 × 18)이고,
  // 쉬지 않고 판 봇에게 **85초**(판의 71%)가 든다. 산소 한 통으로는 610m 라 못 닿는다.
  // ⚠️ **한 판 상한(18개)은 그대로 둔다** — 그것이 분당 밸런스에 걸려 있다
  // (`checkbalance` 의 「분당 수확이 1.5배 안」). 고친 것은 «m 당 요율» 쪽이다.
  // ⚠️ 이 수치는 **`checkdriller` 가 진짜 봇으로 다시 잰다** — 시간으로 «계산»한
  // 잣대는 이 게임에서 두 번 다 틀렸다
  const REWARD_PER = 55;     // 이만큼(m) 내려갈 때마다 재료 하나
  const REWARD_MAX = 18;
  ...
  // ⚠️ **340 에서 올렸다** — 「롱프레스 시 너무 팍팍 부순다」로 신고받은 자리다.
  // 판을 꽉 채운 것이 큰 몫이고(공짜 길이 없어졌다) 여기서 한 번 더 눌러 준다.
  // 손가락을 대고 있는 손이 봇으로 13.6 → **11.6 m/s** 가 됐고, 판마다 흔들리던 폭도
  // 같이 좁아졌다(11.2~15.2 → 10.7~12.2). ⚠️ **이 값을 고치면 상한을 다시 잰다**
  const DIG_MS = 420;        // 색 바위 한 칸을 파는 데 드는 시간
  ...
  // 예전에는 `FILL` 0.86 으로 줄마다 구멍을 뚫어 두었는데, 떠 있는 바위는 첫 낙하 틱에
  // 그대로 무너진다 — **판에 들어서자마자 아무것도 안 눌렀는데 바위가 쏟아지고
  // 넷이 사라졌다** (신고받은 자리다). 게다가 그 구멍이 곧 «공짜 길»이라
  // 손가락을 대고 있으면 파지도 않고 내려가서 「팍팍 부순다」로 읽혔다.
  // 꽉 찬 판에는 **받칠 것이 없는 바위가 하나도 없다** — 그래서 사람이 파기 전에는
  // 한 칸도 안 움직이고, 무너지는 것은 전부 「내가 한 일」이 된다 (미스터 드릴러의 축 그대로다).
  // 첫 줄(`r === 0`)만 비워 두는데 그것이 곧 **지표면**이다 (사람이 서서 시작하는 자리)
  function ensureRows(upto) {
    while (S.rows.length <= upto) {
      const r = S.rows.length;
      const row = new Array(COLS).fill(null);
      if (r === 0) { S.rows.push(row); continue; }             // 지표면 — 여기서 출발한다
      ...
      // 에어 캡슐 — 줄마다 많아야 하나. ⚠️ **빈 칸이 없으니 칸 하나를 «대신» 차지한다**
      // (미스터 드릴러의 캡슐도 판 안에 박혀 있고, 파 들어가면서 줍는다).
      // 예전에는 「빈 칸에만 놓는다」였는데 판이 꽉 차면 **캡슐이 영영 안 나온다** —
      // 산소가 이 게임의 상한을 지키므로 그것이 곧 「2분을 다 쓸 길이 없어지는」 것이다
      if (Math.random() < CAP_RATE) {
        row[Math.floor(Math.random() * COLS)] = { k: 'air', s: Math.random() * 99 };
      }
      S.rows.push(row);
    }
  }
