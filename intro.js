...
(function () {
  ...
  // 목 — **턱 그림자를 같이 깐다.** 없으면 얼굴과 목이 같은 살색 덩어리로 붙어
  // 턱선이 사라진다 (「얼굴 실루엣이 이상해 보인다」).
  //
  // ⚠️ **얼굴 «위»가 아니라 목 «위»에 그린다.** 예전에 얼굴 타원 안쪽, 입 바로 밑에
  // 그늘을 깔았다가 **수염처럼 보여서** 뺐다(`princessHead` 주석). 여기 그늘은
  // 목에 지는 것이고, 얼굴이 그 위에 덮이므로 **턱 밑 초승달만** 남는다.
  // ⚠️ 목 모양으로 잘라야 한다 — 안 자르면 타원이 목 밖 배경으로 삐져나온다.
  const PZ_NECK = 'M136,190 L164,190 L164,212'
    + ' C168,220 172,226 175,240 L125,240 C128,226 132,220 136,212 Z';
  function princessNeckSkin() {
    return `<defs><clipPath id="pzNeckClip"><path d="${PZ_NECK}"/></clipPath></defs>
      <path d="${PZ_NECK}" fill="${SKIN}"/>
      <ellipse cx="150" cy="209" rx="26" ry="12" fill="${SKIN_SH}" opacity="0.5"
        clip-path="url(#pzNeckClip)"/>`;
  }
  ...
  // ─── 공주의 뒷머리 — **한 곳에서만 그린다** ──────────────────
  //
  // ⚠️ 예전에는 이 열 줄이 **두 군데에 똑같이** 있었다 (인트로의 야식 장면과
  // `princessHead`). 턱을 감싸던 것을 고치면서 한쪽만 고쳤고, 그런데도 화면이
  // 바뀌길래 다 된 줄 알았다 — 다른 장면은 그대로였다. 그래서 함수로 모았다.
  //
  // ⚠️ **얼굴 타원보다 «아래»로 내려가면 안 된다.** 예전에는 cy176 ry38 이라
  // 밑끝이 214 로 얼굴(cy180 ry33 · 밑끝 213)보다 낮았고 게다가 더 넓어서,
  // **턱 밑에 갈색 초승달**이 남았다 — 머리가 턱을 감싸 「수염·턱끈」처럼 보였다.
  // 지금은 cy174 ry32(밑끝 206)라 볼 아래에서 얼굴 뒤로 숨는다.
  //
  // ⚠️ **그 대신 옆 가닥이 턱선을 이어받아야 한다.** 뒷머리를 올린 만큼
  // 턱 옆(y 206 언저리)이 비어, 얼굴과 드레스 어깨 사이로 **배경이 2px 비쳤다**
  // (`checkavatar` 의 「초상화」가 잡았다). 가닥의 안쪽 변을 얼굴 쪽으로 밀어
  // 그 자리를 메운다 — 겹치는 부분은 얼굴이 위에 그려져 어차피 안 보인다.
  function princessHairBack() {
    return `<ellipse cx="150" cy="174" rx="40" ry="32" fill="${HAIR}"/>
      <path d="M112,176 C106,214 114,244 128,246 C136,222 134,196 132,178 Z" fill="${HAIR}"/>
