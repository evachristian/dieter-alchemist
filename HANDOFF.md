...
- **다리는 가늘어져도 붙어 있다.** 허벅지·종아리 굵기 배율의 축을 파츠 가운데에서
  **안쪽 변**으로 옮겼다 (`THIGH_IN` · `CALF_IN`). 가운데였을 때는 허벅지 60% 에서
  다리 사이 틈이 4px → 16px 로 벌어졌다. 발·구두도 종아리를 따라간다(`footX`).
  `checkavatar` 의 **「다리 사이 틈」**이 배율 6단계를 돈다
