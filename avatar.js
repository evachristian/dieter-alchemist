  ...
  function hairFront(kind, c) {
    switch (kind) {
      ...
      default:            // 일자뱅 (스트레이트) — **인트로 공주와 같은 앞머리**
        ...
        //
        // ⚠️ **헤어라인(안쪽 호)만은 공주보다 2.7px 깊다** (40.3 이 아니라 43).
        // 공주 그대로 40.3 에 두면 `checkavatar` 의 「정수리에 살색」이 잡는다 —
        // 그 검사는 얼굴 꼭대기부터 8px 을 보는데, 커튼뱅의 가르마가 y40 에서 열려
        // **정수리가 갈라져 보이던** 버그를 잡는 창이라 좁힐 수가 없다
        // (그 버그의 살색과 여기 이마의 살색이 같은 높이에서 시작한다).
        // 아바타는 공주보다 얼굴이 세로로 길고 눈이 낮게 붙어 이마가 원래 더 넓으므로,
        // 헤어라인을 조금 내리는 쪽이 오히려 공주에 가깝게 보인다
