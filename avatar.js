...
(function () {
  ...
  // 목 굵기를 통째로 줄이는 몫. **한 곳이라 목·옷깃 입구(`neckCut`)·받침의 감싸는
  // 폭(`NECK_HUG`)이 같이 따라온다.** 0.74 는 「제곱 스케일」을 펴면서 바디파츠 최소의
  ...
  function neckGusset(color, neck, neckKind) {
    // ── ⚠️ **옷을 입었으면 이 자리를 메우지 않는다.** 목 옆에 «무엇을 그리든» 목이
    // 굵어 보인다 — 세 번 신고받고 세 판을 그려 보고 내린 결론이다:
    ...
    // 그래서 **맨몸과 «똑같이» 둔다** — 목 옆에 배경이 비치는 것이 이 그림에서
    // 맞는 모양이다 (`86a649a` 가 그랬고, 「턱밑 머리」는 오히려 그 틈을 4px 이상
    // 요구한다). 옷은 **제 넥라인**(`NECK_CUT`)을 그대로 그린다.
    ...
    if (color !== SKIN && (!ncK || ncK.pad != null)) return '';
    ...
    // 맨몸에서는 이 조각도 살색이라 목과 한 덩어리로 보여서, 바디파츠를 최소로 내리면
    // **목이 굵어 보였다** (「목이 좀 두꺼운 것 같아」로 신고받았다).
    // 비율로 두면 어느 크기에서나 같은 모양이고, **100% 에서는 정확히 5px 이라
    // 한 글자도 안 바뀐다**
    // ⚠️ **맨몸일 때만 폭을 «목»에 붙인다** (`NECK_HUG`). 이 조각은 역할이 둘이다:
    //   · 옷을 입었으면 **깃**이다 — 턱선의 현을 그대로 써서 턱 밑을 끝까지 덮어야
    //     한다. 좁히면 얼굴 아래 호와 어깨 사이에 **배경이 쐐기처럼** 남는다
    //     (좁혀 놓고 재 보니 얼굴 125·150% 에서 좌우 5~6px 이 비쳤다 — 이 조각을
    //     애초에 만들게 한 「공주 옷 어깨 선이 어긋난다」 그 자리다)
    //   · 맨몸이면 **살색이라 목과 한 덩어리로 보인다** — 얼굴 반폭의 0.52배가 목
    //     옆에 깔려서 **진짜 목(±3.35)이 ±8.5 짜리 기둥으로** 읽힌다
    //     (「목이 두껍다」로 신고받았다). 그 바깥은 덮을 것이 없는 **빈 자리**이고,
    //     거기에 배경이 비치는 것은 이 그림에서 원래 맞는 모양이다
    //     (`ART_POLICY.md` · 「턱밑 머리」가 그 틈을 오히려 4px 이상 요구한다)
    // 가르는 줄은 **색 하나**다 — `clothColor` 는 입은 것이 없을 때만 `SKIN` 이다.
    // 옆변이 어깨까지 흘러내리는 것(`sx`)은 둘 다 그대로다
    const rise = ry * GUSSET_RISE_K;
    const jaw = rx * Math.sqrt(Math.max(0, 1 - ((ry - rise) / ry) ** 2));
    const xc = color === SKIN ? Math.min(jaw, half * NECK_HUG) : jaw;
  ...
  // 받침의 윗변이 목보다 얼마나 넓은가 — 목을 «감싸는» 몫이다. **맨몸에서만 쓴다.**
  // 1 이면 목과 딱 붙어 경계에 배경이 실오라기처럼 비칠 수 있어 15% 만 물린다
  const NECK_HUG = 1.15;
  ...
  function build(outfit, body, tune) {
    ...
    // 깃이 «낮은» 옷인가 — 판정은 `NECK_CUT` 표 하나다 (`pad == null` 이 곧
    // 「목까지 올라온다」). 깃이 높으면 옷이 제 힘으로 목까지 닿으므로 살을 안 깐다
    const lowNeck = !!(NECK_CUT[clothNeck] && NECK_CUT[clothNeck].pad != null);
    ...
    const layers = [
      ...
      // ⚠️ **옷을 입어도 턱 밑의 «살»은 맨몸과 같아야 한다** (2026-09-24 ·
      // 「목부분이 옷 벗었을 때랑 옷 입었을 때랑 다른 것 같다」로 신고받았다).
      // 깃이 낮은 옷에서 이 조각을 통째로 안 그리고 있었는데, 그 자리는 옷이
      // **닿지도 못하는 자리**다 — 옷의 어깨선(`CLOTH_TOP_Y`)은 턱보다 13px 아래라,
      // 턱과 어깨 사이가 통째로 **투명**으로 남아 방 벽(베이지)이 비쳤다.
      // 재 보면 바디파츠 100%·날씬 y102 에서 맨몸은 반폭 13.3 의 살인데 옷을 입으면
      // **0** 이었다 — 목만 길쭉한 막대로 보이던 것이 그 몫이다.
      // ⚠️ **옷 «밑»이라야 한다.** 옷 위에 두면 어깨 위로 살이 퍼져 드레스 위에
      // 맨어깨가 얹힌다 (그려 보고 되돌린 자리다). 밑에 두면 옷이 제 모양대로
      // 덮고 **옷이 못 덮는 만큼만** 남으므로, 옷마다 따로 맞출 것이 없다.
      // ⚠️ 색은 **살색**이다 — 옷 색으로 메우면 옷깃이 턱까지 올라와 목 양옆에
      // 기둥이 선다 (「초록 기둥이 얼굴까지 이어져 올라온다」 · 아래 `neckGusset` 맨 앞).
      // 맨몸은 여기서 안 그린다 — 예전 자리(옷보다 앞)에서 그대로 그려서
      // **한 픽셀도 안 바뀐다**
      B(clothColor === SKIN || !lowNeck ? '' : neckGusset(SKIN, neck, clothNeck)),
