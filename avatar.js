...
(function () {
  ...
  //
  // ⚠️ **긴 생머리만 넓다** (`CROWN_LONG`). 인트로 공주와 나란히 재 보니 머리 반폭은
  // 둘 다 40 으로 «같았는데», 공주는 얼굴 반폭이 33.75 이고 아바타는 **귀가 튀어나와**
  // 37.88 이었다 — 그래서 볼 높이에서 얼굴 옆으로 나온 몫이 공주 5.50 · 아바타 1.00 이라,
  // 아바타만 머리가 얼굴에 딱 붙어 «뒷머리가 안 솟은» 것으로 보였다.
  // 귀를 깎는 대신 뒤통수를 넓혀 그 몫을 맞춘다 (46.5 → 5.75).
  // 시작 착장이 `hair_long` 이라 여기만 고치면 처음 진입한 플레이어가 공주와 같아진다.
  const CROWN_LONG = { rx: 44.7, ry: 42 };
  function hairBack(kind, c) {
    const s = shade(c, 22);
    const crownOf = (rx, ry) => `<ellipse cx="100" cy="63" rx="${rx}" ry="${ry}" fill="${c}"/>`;
    ...
    switch (kind) {
      ...
      default: // long
        // ⚠️ **옆 가닥도 같이 넓힌다.** 뒤통수만 넓히면 가닥이 그보다 좁아,
        // 귀 높이에서 윤곽이 «쏙» 들어가 머리가 뒤로 꺾인 것처럼 보인다
        // (공주는 뒤통수 39.75 · 가닥 39 로 거의 같아서 매끈하게 떨어진다)
        return crownOf(CROWN_LONG.rx, CROWN_LONG.ry) +
          `<path d="M55.8,66 C53.8,104 58,152 75,153 C66,120 66,92 72,72 Z" fill="${c}"/>
           <path d="M144.2,66 C146.2,104 142,152 125,153 C134,120 134,92 128,72 Z" fill="${c}"/>`;
    }
  }
