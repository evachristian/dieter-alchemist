...
(function () {
  ...
  // ⚠️ **팔에도 몫을 붙였다** (`arm: 3.0`). 슬라이더 상한이 150% 뿐이라 그냥 곱하면
  // 끝까지 올려도 «가는 막대»를 못 벗어난다 — 통통한 몸 옆에서 팔만 실처럼 남았다.
  // 100% 를 넘는 만큼에만 3배로 얹는다. 100% 이하는 한 톨도 안 변한다.
  // ⚠️ **한 번 4.4 까지 갔다가 3.0 으로 내려왔다.** 4.4 는 150% 에서 두께가
  // 13.5 → 38.8px(2.87배)이었는데 「두 배는 무리수였다」로 되돌아왔다 —
  // 지금은 **30.8px** 이다 (38.8 에서 −21%).
  //
  ...
  const TUNE_GAIN = { thigh: 0.92, hip: 0.42, calf: 0.92, arm: 3.0 };
