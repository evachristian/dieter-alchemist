  ...
  function hairFront(kind, c) {
    switch (kind) {
      ...
      default:            // 일자뱅 (스트레이트) — **인트로 공주와 같은 앞머리**
        ...
        // ⚠️ **바깥 호는 «손으로 그린 곡선»이 아니라 뒤통수와 같은 타원의 호다.**
        // 예전에는 3차 곡선으로 어림해 그렸는데, 그러면 관자놀이 근처에서 앞머리가
        // 뒤통수보다 «살짝 넓어지는» 구간이 생겨 실루엣에 **턱이 진다**
        // (y32 에서 앞머리 74.8 · 뒤통수 85.1 → y63.6 에서 뒤집힌다. 두 선이
        //  비스듬히 만나니 옆라인에 각이 남았다).
        // 같은 중심·같은 rx 에 **ry 만 늘린 타원**(63.64−23.3 = 40.34)으로 두면,
        // 뒤통수를 위쪽에서 통째로 품으면서 y63.64 에서 폭이 정확히 같아지고
        // 접선까지 나란해진다 — 이음매가 사라진다.
        // 값은 전부 뒤통수(`CROWN_LONG`)에서 나오므로 **따로 흔들리지 않는다**
        return (() => {
          const cy = CROWN_LONG.cy, rx = CROWN_LONG.rx, ry = cy - 23.3;
          const L = (100 - rx).toFixed(2), R = (100 + rx).toFixed(2), Y = cy.toFixed(2);
          // 안쪽 호(헤어라인)는 **관자놀이에서 «세워서» 얼굴선에 붙인다.**
          // 완만하게 내려오면 얼굴 윤곽과 비스듬히 만나 그 자리에 각이 남는다
          // (얼굴 타원의 그 높이 기울기는 「왼쪽으로 1 갈 때 아래로 2.2」다).
          // 끝에서 «1 : 3.5» 로 세워 두면 사람 눈에는 이어진 선으로 보인다.
          // 남는 조각(x ${L}~67)은 뒤통수 안쪽이라 안 보인다
          return `<path d="M${L},${Y} A${rx},${ry.toFixed(2)} 0 0 1 ${R},${Y}
            L133,63.6 C130,53 115,46.5 100,46.5 C85,46.5 70,53 67,63.6 Z"
            fill="${c}"/>`;
        })();
    }
  }
