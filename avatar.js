...
(function () {
  ...
  // ─── 그림 상자 ─────────────────────────────────────────────
  //
  // ⚠️ **가로를 200 → 244 로 넓혔다.** 통통 최대는 (그때는) 몸이 가로로 1.36배가 되는데
  // (`bodyScaleX` — 지금은 부위마다 `FAT_W` 로 붙고 통째 배율은 1.12 다), 200 짜리 상자에서는 허벅지 하나가 폭의 90% 를 먹어 **엉덩이가
  ...
  // `viewBox` 가 200 인데 **통통 최대는 이미 191.5 까지 차 있었다** — 그때는 몸무게가
  // 몸 전체를 가로로 1.36배 늘렸기 때문이다(`bodyScaleX` · 지금은 부위별 `FAT_W`).
  ...
  // 슬라이더가 만드는 살의 양 (몸무게는 안 들어 있다)
  function gainOf(tune, k) {
    const v = tuneOf(tune, k), g = TUNE_GAIN[k];
    return (g == null || v <= 1) ? v : 1 + (v - 1) * g;
  }
  // ─── 살은 «부위마다 다르게» 찐다 ────────────────────────────
  //
  // ⚠️ 예전에는 몸무게(w)가 **몸 전체를 가로로 1.36배** 늘렸다(`bodyScaleX`) — 어깨·
  // 발목·종아리·팔이 허리·배와 똑같은 비율로 굵어져서, 통통한 몸이 「살찐 사람」이
  // 아니라 **가로로 잡아 늘린 날씬한 사람**으로 보였다 (체형 5단계를 나란히 놓으면
  // 실루엣이 그냥 옆으로 넓어질 뿐 «모양»이 안 바뀐다).
  //
  // 지금은 부위마다 몫이 다르다 (외부 바디 엔진 v2 의 표 — 배 .40 · 윗팔 .35 ·
  // 허벅지 .30 · 허리 .26 · 엉덩이 .25 · 종아리 .18 · 어깨 .03 — 를 이 그림의 부위
  // 다섯으로 옮긴 것이다). 살은 허리·배와 허벅지·윗팔에 먼저 붙고, 종아리는 조금,
  // 어깨·발목·무릎은 거의 안 붙는다. 그래야 통통한 몸이 «둥글어진다».
  //   · 몸무게 0(날씬)에서는 어느 부위도 안 움직인다 — 지금까지의 날씬 그림 그대로다
  //   · 남은 몫(`BODY_FAT_X` 0.12)만 예전처럼 몸 전체에 고르게 얹는다 — 어깨·가슴
  //     (이 그림에는 따로 없는 부위)이 그것으로 조금 넓어진다
  //   · 슬라이더 몫(`gainOf`)에 **곱한다** — 둘은 서로 다른 축이다 (슬라이더는 사람이
  //     고른 체격, 몸무게는 물약이 바꾸는 살). 졸업 직후 몸은 슬라이더 150% × 몸무게 1 이다
  //   · ⚠️ 옷도 같은 값을 본다 — `tuneMax` 가 이 몫을 같이 곱하므로 하의·치마의 퍼짐이
  //     굵어진 다리를 따라온다. 몸무게를 슬라이더 값(`tuneOf`)에 섞지 않는 이유는
  //     세이브·화면의 % 표시가 사람이 고른 값 그대로여야 해서다
  // ⚠️ 팔은 0.28 로 시작했다가 **0.18** 로 내렸다 — 졸업 직후(150%)에는 팔에 이미
  // `TUNE_GAIN.arm` 이 얹혀 있어서(1.66배) 거기에 0.28 을 더 곱하면 소시지가 됐다
  const FAT_W = { waist: 0.36, hip: 0.24, thigh: 0.26, calf: 0.14, arm: 0.18 };
  const wOf = tune => Math.max(0, Math.min(1, (tune && Number(tune.w)) || 0));
  const weightK = (tune, k) => 1 + (FAT_W[k] || 0) * wOf(tune);
  // 살의 양 = 슬라이더 몫 × 몸무게 몫. **살의 양을 셈하는 자리에서만** 쓴다
  function fatOf(tune, k) { return gainOf(tune, k) * weightK(tune, k); }
  // build() 가 몸무게를 tune 에 실어 내려보낸다 — 아래의 모든 함수가 tune 하나만
  // 받으므로, 인자를 하나 더 늘리는 대신 여기에 담는다 (사본이라 원본 tune 은 안 바뀐다)
  const withWeight = (tune, w) => Object.assign({}, tune || {}, { w: w });
  ...
  // 슬라이더의 «바닥»을 tuneOf 로 접은 값 (game.js 의 `TUNE_PARTS.min` · `TUNE_MIN` 50)
  // ⚠️ 엉덩이는 눈금이 접히므로(`TUNE_SCALE.hip` 0.5) 20% → 0.1 이다
  const TUNE_LO = { hip: 0.1 };   // 나머지는 0.5
  ...
  // ⚠️ 0.36 → **0.12**. 살이 부위마다 다르게 붙게 되면서(`FAT_W`) 여기에는 어깨·
  // 가슴 몫만 남았다 — 통통 최대의 허벅지 폭은 예전과 거의 같다 (`checkavatar` 「그림 상자」).
  // 0.10 으로 두면 팔을 0.18 로 내린 뒤 어깨가 팔보다 좁아 보여 0.12 로 올렸다
  const BODY_FAT_X = 0.12;
  ...
  // ⚠️ **몸무게 몫까지 곱한다.** 살은 `fatOf`(슬라이더 몫 × 몸무게 몫)로 붙는데
  // 옷이 슬라이더 값만 따르면 통통할 때 굵어진 다리가 치마·바지 밖으로 나온다.
  // 슬라이더 몫은 `tuneOf` 그대로다 — `gainOf` 가 1 아래로 안 깎으니 이쪽이 늘 살 이상이다
  function tuneMax(tune, keys) {
    return keys.reduce((m, k) => Math.max(m, tuneOf(tune, k) * weightK(tune, k)), 1);
  }
  ...
  // 상한은 **슬라이더 몫에만** 건다 — 몸무게 몫까지 깎으면 졸업 직후(150%)에 살이 찌는
  // 만큼이 통째로 사라진다 (팔만 가늘게 남는다)
  const armK = tune => Math.min(gainOf(tune, 'arm'), ARM_K_MAX) * weightK(tune, 'arm');
  ...
  //   G : 밑변 3차 곡선의 제어점을 반폭의 몇 배에 두는가 (아래)
  //
  // ⚠️ **입구가 「납작한 뚜껑」으로 보이던 자리다** (W .72 · D .60 · G .42).
  // 「발등 곡선을 다듬어 달라」는 말을 듣고 셋을 하나씩 떼어 그려 보았는데,
  // **모양을 바꾼 것은 W 와 D 였다** — 밑변의 «생김새»(G)가 아니다.
  //   · **W 를 좁히면 모서리가 타원의 «더 아래»로 내려간다.** 거기서는 신발 옆선이
  //     가팔라서 발등이 그 사이로 파고든 것처럼 보이고, 옆에 신발의 «옆면»이 남는다
  //   · **D 를 깊게 하면 가운데가 더 파여** 밑변이 신발의 입구 선으로 읽힌다.
  //     둘을 같이 하면 입구의 깊이÷폭이 0.120 → 0.152 가 된다 (`checkavatar` 의 「발등 깊이」)
  //   · **G 는 밑변이 얼마나 «평평한가»만 정한다.** 0.42 → 0.15 로 줄이면 바닥이
  //     조금 더 넓고 둥글어질 뿐, 정규화해 재면 프로필이 거의 그대로다 (0.54 → 0.44)
  // ⚠️ **모서리를 «덜 뾰족하게» 만드는 길이 아니다.** 오히려 사이각은 60° → 54° 로
  // 조금 더 날카로워졌는데도 눈에는 낫다 — 뿔로 보이던 것은 각이 아니라 **얕고 넓은
  // 입구**였다. 각을 2차로 «굴려» 보기도 했는데 면이 깎여 **각진 탭**이 되어 버렸다.
  // 숫자를 안 재고 「모서리가 문제」로 짐작했다가 두 번 헛짚은 자리다
  const INSTEP_W = 0.68, INSTEP_D = 0.62, INSTEP_G = 0.15;
  ...
  // ⚠️ **목이 있는 구두에는 굽이 없다** — 하이힐인 것은 유리구두 하나뿐이다
  // (`genwardrobe` 의 `HEEL` 표). 부츠는 목이 발목을 감싸는데(반폭 15.6) 굽을 신어
  // 발만 펌프스 폭(7.8)으로 좁아지면 **목의 안쪽을 발이 못 덮어 배경이 쐐기처럼 비친다**
  // — 그려 보고 알았다. 그래서 여기서도 한 번 더 막고, `checkavatar` 가 표를 본다
  ...
  function renderShoes(it, tune, legHemY) {
    ...
    const foot = (cx) => {
      ...
      if (!rise) {
        ...
        // 제어점 x(`INSTEP_G`)는 «밑변이 얼마나 평평한가»만 정한다 — 위 참고
        const cbY = (instepBot - 0.25 * edge) / 0.75, cbX = iW * INSTEP_G;
        s += `<path d="M${P0(cx - iW)},${P0(edge)}`
          + ` A${P0(FRX)},${P0(FRY)} 0 0 1 ${P0(cx + iW)},${P0(edge)}`
          + ` C${P0(cx + cbX)},${P0(cbY)} ${P0(cx - cbX)},${P0(cbY)} ${P0(cx - iW)},${P0(edge)} Z"`
          + ` fill="${SKIN_SH}"/>`;
      }
  ...
  function hipSolve(tune) {
    // ⚠️ 몸무게도 열쇠에 넣는다 — 안 넣으면 체형이 바뀌어도 옛 엉덩이를 돌려준다
    const key = ['hip', 'waist', 'torso', 'thigh', 'calf'].map(k => tuneOf(tune, k)).join(',') + ',' + wOf(tune);
  ...
  //
  // ⚠️⚠️ **한때 「100% 이하는 건드리지 않는다」였고, 그래서 기본 몸의 허벅지가 가늘었다.**
  // `overOf` 는 100% 이하에서 0 이므로 **100% 허벅지에는 볼록함이 하나도 없었다** —
  // 옆선의 아래 절반이 오목해져서, 재 보면 무릎 바로 위가 **11.0** 인데 장딴지가
  // **14.3** 이었다 (「바디파츠 모두 100% 일 때 허벅지가 너무 가늘다」로 신고받았다).
  // 게다가 100% → 125% 에서 허벅지중÷장딴지가 **1.22 → 1.93** 으로 튀어,
  // 눈금 한가운데에 «허벅지가 갑자기 붙는» 자리가 있었다.
  //
  // 지금은 **바닥(`THIGH_BASE`)이 눈금과 같이 자란다** — 슬라이더 바닥에서 0,
  // 100% 에서 `THIGH_BASE`, 150% 에서 `THIGH_BULGE` 다.
  //   · **50% 는 한 픽셀도 안 바뀐다** (`underOf` 가 0 이다 — 가는 다리는 그대로)
  //   · **150% 도 한 픽셀도 안 바뀐다** (두 항이 만나 `THIGH_BULGE` 가 된다)
  //   · ⚠️ **100~150% «사이»는 같이 올라간다.** 100% 를 올리면서 150% 를 그대로 두면
  //     그 사이가 따라 올라가는 것이 매끄러운 쪽이다 — 계단으로 두면(`Math.max`)
  //     100~125% 가 한 값에 붙어 눈금이 「안 먹는」 구간이 된다
  // ⚠️⚠️ **값은 «재서» 골랐다 — 근거가 셋이고 셋이 같은 자리를 가리킨다.**
  // 잣대는 허벅지중÷장딴지(`checkavatar` 의 「허벅지↔장딴지」가 재는 그 값)다:
  //   · 옛 코드  1.02 · 1.04 · **1.06** · 1.44 · 1.86  ← 칸마다 .02 .02 **.38** .42
  //   · 1.0      1.02 · 1.16 · **1.42** · 1.64 · 1.86  ← 칸마다 .14 .26 .22 .22
  //   ① **눈금이 칸마다 같은 일을 한다** (제일 큰 칸 ÷ 제일 작은 칸 1.9배 · 옛것은 21배)
  //   ② 100% 가 사람 다리의 비(허벅지 가운데 = 장딴지의 1.4~1.5배)에 앉는다
  //   ③ **옛 125%(1.44)보다 굵어지지 않는 제일 큰 값**이다 — 1.2 는 1.51 로 넘어선다
  // ⚠️ 1.6 부터는 **허벅지 가운데가 윗머리보다 굵어져** 통나무가 된다 (29.3 > 28.8)
  const THIGH_BULGE = 1.4;
  const THIGH_BASE = 1.0;
  const underOf = (tune, k) => { const lo = TUNE_LO[k] || 0.5;
    return Math.max(0, Math.min(1, (tuneOf(tune, k) - lo) / (1 - lo))); };
  const thighBul = tune =>
    THIGH_BASE * underOf(tune, 'thigh') + (THIGH_BULGE - THIGH_BASE) * overOf(tune, 'thigh');
  ...
  // 목 굵기를 통째로 줄이는 몫. **한 곳이라 목과 옷깃 입구(`neckCut`)가 같이
  // 따라온다.** 0.74 는 「제곱 스케일」을 펴면서 바디파츠 최소의
  // 목이 3.35 → 4.52 로 **굵어진 것을 되돌리는** 값이다 (그대로 3.35 가 된다).
  // 기본(100%)은 9.04 → 6.69 로 26% 가늘어진다 — 「전 크기 다 가늘게」로 정한 것이다
  const NECK_SLIM = 0.74;
  ...
  function neckGusset(color, neck, neckKind) {
    // ── ⚠️⚠️ **이 조각은 «깃이 높은 옷»의 것이다. 맨몸에도, 깃이 낮은 옷에도 안 그린다.**
    //
    // 목 옆에 «무엇을 그리든 목이 굵어 보인다» — 네 판을 그려 보고 네 번 다 신고받았다:
    //   · 옷 색으로 메움 → 옷깃이 턱까지 올라와 목 양옆에 기둥이 선다
    //     (「초록 기둥이 얼굴까지 이어져 올라온다」)
    //   · 위를 살색으로 덮음 → 턱과 목이 한 덩어리가 되어 굵어 보이고, 조각이
    //     어깨선 아래까지 내려가 **옷의 제 넥라인까지 덮었다**
    //     (「목 두께에 비해 동그랗게 파인 부분이 너무 작다」)
    //   · 턱 밑 그늘색으로 덮음 → 그래도 「옷을 입으면 양옆으로 목이 더 생긴다」
    //   · **맨몸에만 살색으로 두기**(`NECK_HUG` · 2026-09-24 에 지웠다) →
    //     ① 옷을 입은 몸과 달라 보이고 ② 맨몸의 목이 굵어 보인다.
    //     목의 그라데이션은 «진짜 목 폭»(`half`)으로 서는데 그 둘레의 받침은 평평한
    //     살색이라, **그림자가 목보다 좁은 «네모 블록»으로 떠 보였다**
    //     (「옷 벗었을 때 굵어지고 그림자가 목 두께와 안 맞아」로 신고받았다)
    //
    // ⚠️ **답은 「목만 그린다」다.** `neckShape` 가 이미 제대로 생겼다 — 턱부터
    // 곧은 기둥이고 어깨(`torsoTopY − 1`) 바로 위에서 벌어진다. 받침은 그 위에
    // **턱에서부터 벌어지는 두 번째 자락**을 덧대어 목을 원뿔로 만들고 있었다.
    // 목 옆에 배경이 비치는 것이 이 그림에서 맞는 모양이다 (`86a649a` 가 그랬고,
    // 「턱밑 머리」는 오히려 그 틈을 4px 이상 «요구»한다).
    // ⚠️ **깃이 높은 옷(터틀넥·폴로)만 메운다** — 거기서는 옷깃이 목까지 올라오는
    // 것이 맞는 모양이라, 안 메우면 옷과 목 사이로 배경이 비친다.
    // 판정은 **`NECK_CUT` 표 하나**다 (`pad == null` 이 곧 「목까지 올라온다」) —
    // **색으로 가르지 않는다**: 맨몸(`SKIN`)도 여기서 걸러진다(`neckKind` 가 없다)
    const ncK = NECK_CUT[neckKind];
    if (!ncK || ncK.pad != null) return '';
    ...
    if (!(rx > half) || !(ry > 0)) return '';
    // ── 윗변은 **턱선**(얼굴 타원의 아래 호)이다. 턱보다 `GUSSET_RISE` 위에서 자른다 —
    // 끝까지 따라가면 옷깃이 볼 옆까지 기어올라 «얼굴을 감싼 깃»이 된다
    const cy = chin - ry;
    // ⚠️ **높이는 얼굴을 따라간다** (`GUSSET_RISE_K` — px 가 아니라 «비율»이다).
    // 5px 로 박아 두었더니 **머리가 작아질수록 이 조각이 상대적으로 넓어졌다**:
    // 턱은 타원의 맨 아래라 5px 위의 현(弦) 폭은 `rx·√(1−((ry−5)/ry)²)` 인데,
    // ry 가 34(기본) → 17(최소)로 줄면 그 몫이 얼굴 반폭의 **0.52 → 0.71 배**가 된다.
    // 비율로 두면 어느 크기에서나 같은 모양이고, **100% 에서는 정확히 5px 이다**
    // ⚠️ **폭은 턱선의 현을 «끝까지» 쓴다.** 좁히면 얼굴 아래 호와 어깨 사이에
    // **배경이 쐐기처럼** 남는다 (좁혀 놓고 재 보니 얼굴 125·150% 에서 좌우
    // 5~6px 이 비쳤다 — 이 조각을 애초에 만들게 한 「공주 옷 어깨 선이 어긋난다」
    // 그 자리다). 깃이 높은 옷 하나만의 조각이라 **역할이 하나뿐이고**, 그래서
    // 「맨몸이면 좁힌다」(`NECK_HUG`)가 필요 없어졌다 — 지웠다
    const rise = ry * GUSSET_RISE_K;
    const xc = rx * Math.sqrt(Math.max(0, 1 - ((ry - rise) / ry) ** 2));   // 턱선의 현
    // ⚠️ **끝점은 반드시 타원 «위»에 둔다.** 폭만 줄이고 높이를 그대로 두면 끝점이
    // 타원 밖이 되어 **SVG 가 타원을 다시 맞춘다** — 호가 납작해져 턱을 덮는다
    // (예전에 「캡만 씌웠다가」 겪은 그 사고다). 폭을 정하고 «높이를 거기서 구한다» —
    // `xc` 가 턱선의 현 그대로이면 이 식은 `chin - rise` 와 같은 값이라, **옷을 입은
    // 몸은 한 글자도 안 바뀐다** (그린 path 를 대조했다)
    const cut = cy + ry * Math.sqrt(Math.max(0, 1 - (xc / rx) ** 2));
    // ── 옆변은 **어깨로 흘러내리는 선**이다.
    //
    ...
    // ⚠️ **옆변은 «가다가» 벌어져야 한다** (`GUSSET_FLOW`). 제어점을 시작점과 같은 x 에
    // 두면 접선이 «수직»이라, 띠가 어깨 바로 위까지 같은 폭으로 내려가다가 끝에서만
    // 꺾인다. 그 폭은 머리를 따라 줄어드는데(`xc`·`half` 둘 다 `hk` 배) 높이는 거의
    // 그대로라(턱~어깨 24.8 → 28.4), **바디파츠 최소에서 띠가 «끈»이 된다** —
    // 폭÷높이가 기본 0.293 → 최소 0.158 로 반이 된다 (「목 양옆의 초록 기둥이
    // 이상하다」로 신고받았다). 제어점을 어깨 쪽으로 밀면 턱 밑에서부터 고르게
    // 벌어져 «옷의 어깨»로 읽힌다. 도착 접선은 그대로 수평이라 어깨와의 이음매는
    // 안 바뀌고, x 는 세 점의 볼록껍질 안이라 **어깨 끝(`sx`)을 넘을 수가 없다**
    // ⚠️ **이 값을 지키던 검사(「옷깃 띠」)는 2026-09-24 에 걷었다.** 그것이 막던
    // 사고(띠가 «끈»이 된다)는 **깃이 낮은 옷의 받침**의 것이었는데 그 받침이
    // 없어졌다 — 깃이 높은 옷에서는 옷깃이 목을 통째로 덮어 «띠»가 아예 없고,
    // 몸이 커지면 턱선의 현이 어깨보다 넓어져(`xc ≥ sx`) 이 값이 하는 일이 없다.
    // 남은 일은 작은 몸에서 옆선이 수직으로 안 떨어지게 하는 것뿐이다
  ...
  // ⚠️ **px 가 아니라 얼굴 세로 반지름에 대한 «비율»이다** — px 로 박으면 머리가
  // 작아질수록 이 조각이 상대적으로 넓어져 목이 굵어 보인다 (위 `neckGusset`).
  // 기본 얼굴(ry 34)에서 정확히 5px 이라 100% 는 예전과 한 글자도 안 다르다
  const GUSSET_RISE_K = 5 / 34;
  // 옆변이 어깨까지 가는 길의 «몇 분의 몇»에서 벌어지기 시작하는가 (0 이면 수직으로
  // 떨어지다 끝에서만 꺾인다 — 작은 몸에서 띠가 끈이 되던 값이다). 절반이 기준이다.
  // ⚠️ **이제 깃이 높은 옷에서만, 그것도 작은 몸에서만 일한다** (위 `neckGusset`)
  const GUSSET_FLOW = 0.5;
  // 검사기가 「턱선 바로 아래」를 찾을 때 쓰는 기본 크기의 값 (px)
  const GUSSET_RISE = 5;
  ...
  //
  // ⚠️ **한 번 옮겼다가 되돌린 값이다.** 카와이 참고 그림의 결(「눈 둘 사이에 작은
  // 입이 바짝 붙는다」)을 따라 눈을 26 → 30 으로 벌리고 입을 89 → 84 로 올렸는데,
  ...
  const aEye = {
    ...
    // ⚠️ **`big` 을 그대로 키우지 않는다** — 「아하」도 그것을 쓰는데 거기까지
    // 커지면 안 된다. 「궁금」만 1.25배짜리를 따로 쓴다 (흰자·눈동자·하이라이트 전부)
    bigx:  x => `<ellipse cx="${x}" cy="${AV.Y}" rx="10" ry="12" fill="#fff"/>`
              + `<ellipse cx="${x}" cy="${AV.Y + 0.75}" rx="6.75" ry="8.25" fill="${AV.INK}"/>`
              + `<circle cx="${x + 2.5}" cy="${AV.Y - 3.25}" r="2.75" fill="#fff"/>`
              + `<circle cx="${x - 2.5}" cy="${AV.Y + 5}" r="1.38" fill="#fff" opacity="0.85"/>`,
    ...
    low:   x => `<ellipse cx="${x}" cy="${AV.Y}" rx="8" ry="9.5" fill="#fff"/>`
              + `<ellipse cx="${x}" cy="${AV.Y + 3.25}" rx="5.5" ry="6" fill="${AV.INK}"/>`
              + `<circle cx="${x + 2}" cy="${AV.Y + 1}" r="1.88" fill="#fff"/>`,
    up:    x => `<ellipse cx="${x}" cy="${AV.Y}" rx="8" ry="9.75" fill="#fff"/>`
              + `<ellipse cx="${x}" cy="${AV.Y - 3.25}" rx="5.5" ry="6" fill="${AV.INK}"/>`
              + `<circle cx="${x + 2}" cy="${AV.Y - 5.5}" r="1.88" fill="#fff"/>`,
    side:  x => `<ellipse cx="${x}" cy="${AV.Y}" rx="8" ry="9.25" fill="#fff"/>`
              + `<ellipse cx="${x + 3}" cy="${AV.Y}" rx="5.25" ry="6" fill="${AV.INK}"/>`
              + `<circle cx="${x + 4.5}" cy="${AV.Y - 2.25}" r="1.75" fill="#fff"/>`,
  ...
  const aMouth = {
    ...
    // 인트로 공주의 웃는 입 (`M142,188 Q150,196 158,188`)을 얼굴 포개기 배율로 옮긴 것.
    // 아바타의 `smile` 보다 **넓고 깊다** (폭 10 → 15.6 · 깊이 2.3 → 4.2)
    psmile: `<path d="M${AV.MX - 7.8},${AV.MY} q7.8,8.5 15.6,0" stroke="${AV.LIP}" stroke-width="2.4" fill="none" stroke-linecap="round"/>`,
  ...
  const AV_FACE = {
    ...
    curious: { e: 'bigx',    m: 'ohh',    bL: 'up', bR: 'flat' },
  };

  // ─── 코 ────────────────────────────────────────────────────
  // ⚠️ **코를 그리지 않는다.** 한동안 콧등의 그늘을 선 하나로 옅게 찍어 두었는데
  // (`M98.8,81.6 q1.8,2 3,0.4` · opacity 0.6), **인트로 공주에게는 코가 없다**
  // (`intro.js` 의 princessFace — 눈과 입뿐이다). 같은 사람이라 한쪽에만 있으면
  // 튜토리얼을 막 마치고 마이 룸에 들어섰을 때 또 다른 얼굴이 된다.
  // 귀를 뺀 것과 같은 이유이고, 치비 얼굴은 원래 코가 없어도 성립한다.
  //
  // ⚠️ **표정마다 지우지 않는다.** 코는 `head()` 한 곳에서 나오므로 여기서 빠지면
    ...
    return `
      <g data-part="head">
        ...
        <!-- 볼터치 — 인트로 공주의 것을 그대로 옮겼다 (intro.js 의 i-blush:
             ...
             볼터치가 더 위에 있어 그 높이의 얼굴이 넓지만 아바타는 아래라 좁다.
             그래서 «얼굴 옆선에서 남는 여백»을 공주와 같게(1.9px) 맞춰 81/119 로 넣는다.
             세로는 «입에서 잰다» —
             공주는 볼터치가 입보다 2 위(186 vs 188)라 아바타도 MY-2.5 다.
             세로까지 배율로 옮기면 y76 이 되어 «눈 뒤로» 들어간다
             (공주는 눈이 더 위에 붙어 있어서 그 자리가 볼이지만, 아바타는 눈이다) -->
  ...
  // 뒷머리의 «뒤통수» — 얼굴 뒤로 비치는 덩어리.
  //
  // ⚠️ **긴 생머리는 인트로 공주의 도형을 «그대로 옮겨 온 것»이다** (`CROWN_LONG`).
  //
  // 두 얼굴 타원을 포개는 배율이 유일한 원본이다:
  //     x = 100 + (x공주 − 150) × 33/34      (얼굴 반폭 34 → 33)
  //     y =  70 + (y공주 − 180) × 35/33      (얼굴 반높이 33 → 35)
  // 공주의 뒤통수(cx150 cy174 rx40 ry32)를 이 식에 넣으면 아래 값이 나온다.
  // **수치를 손으로 흔들지 말 것** — 고칠 일이 있으면 `intro.js` 의 `princessHairBack`
  // 을 고치고 여기에 다시 옮긴다. `checkavatar` 의 「뒷머리」가 둘을 견준다.
  //
  // ⚠️ **예전에는 뒤통수만 44.7 로 넓혀 두었다.** 아바타에는 귀(cx67·133 rx6)가 있어
  // 얼굴 반폭이 39 인데 공주는 33.75 라, 같은 폭으로는 귀가 머리 밖으로 튀어나왔다.
  // 그때는 뒤통수를 넓혀 덮었는데 **그러면 공주보다 머리가 크다** — 나란히 놓으면
  // 다른 사람으로 보였다. 지금은 폭을 공주에 맞추고, 대신 **옆 가닥을 얼굴 «앞»에**
  // 둔다(`hairSide`) — 가닥이 귀를 덮으므로 넓힐 이유가 없어진다.
  const CROWN_LONG = { cy: 63.64, rx: 38.82, ry: 33.94 };
  function hairBack(kind, c) {
    const s = shade(c, 22);
    const crownOf = (rx, ry, cy) => `<ellipse cx="100" cy="${cy || 63}" rx="${rx}" ry="${ry}" fill="${c}"/>`;
    // ⚠️ **뒤통수는 여섯이 다 같은 타원이다** (`CROWN_LONG`).
    // 예전에는 긴 생머리만 이 타원을 쓰고 나머지 다섯은 `crownOf(40, 42)` 였는데,
    // 그것은 밑끝이 y105(턱)까지 내려와 **턱 밑에서 목을 파고들었다** —
    // 얼굴이 끝나는 자리 아래에도 머리가 남아 목 옆에 브라운이 닿았다.
    // 이 타원은 밑끝이 y97.6 이라 얼굴 뒤에서 끝난다.
    // 정수리는 앞머리(`BANG_OUT`)가 만들므로 낮아져도 머리가 납작해지지 않는다
    const crown = crownOf(CROWN_LONG.rx, CROWN_LONG.ry, CROWN_LONG.cy);
    switch (kind) {
      ...
      default: // long
        // **뒤통수만 여기 있다.** 옆 가닥은 얼굴 앞(`hairSide`)으로 갔다 — 귀를 덮어야
        // 공주와 같은 윤곽이 나온다. 뒤에 두면 귀가 가닥을 뚫고 나온다
        // 옆 가닥도 공주의 것을 그대로 옮겼다
        // (`M112,176 C106,214 114,244 128,246 C136,222 134,196 132,178 Z` · 오른쪽은 x=100 대칭).
        // ⚠️ **얼굴보다 «뒤»에 둔다** — 앞에 두면 안쪽 변(x82.5)이 얼굴 안으로 15px
        // 들어와 있어서 눈과 볼을 덮는다. 공주도 뒤에 두고 겹치는 부분을 얼굴로 가린다
        return crownOf(CROWN_LONG.rx, CROWN_LONG.ry, CROWN_LONG.cy) +
          `<path d="M63.1,65.8 C57.3,106.1 65.1,137.9 78.7,140 C86.4,114.6 84.5,87 82.5,67.9 Z" fill="${c}"/>
           <path d="M136.9,65.8 C142.7,106.1 134.9,137.9 121.3,140 C113.6,114.6 115.5,87 117.5,67.9 Z" fill="${c}"/>`;
    }
  }

  // 앞머리의 **바깥 윤곽은 다섯이 다 같다.** 그것은 앞머리의 모양이 아니라
  // «머리의 꼭대기»라서다 — 스타일을 가르는 것은 안쪽 변(헤어라인)뿐이다.
  //
  // ⚠️ **이걸 안 맞춰 놨다가 버그를 냈다.** 「기본」만 인트로 공주의 선(꼭대기 y23.3)
  // 으로 올려 두고 나머지 넷은 y29~30 에 남겨 뒀는데, 긴 생머리의 뒤통수는
  ...
  function hairFront(kind, c, back) {
    ...
    switch (kind) {
      ...
      case 'side':        // 사이드뱅 — 한쪽으로 비스듬히 넘긴 가르마.
        ...
        return `<path d="${BANG_OUT}
            C131,52 126,47 118,48 C104,50 92,58 82,60 C75,61 70,58 67,63.6 Z" fill="${c}"/>`;
      case 'curtain':     // 커튼뱅 — 가운데를 열고 양옆으로 갈라 내린다.
        // 가르마는 **헤어라인 근처(y50)에서** 열어야 한다. 예전에는 y40 에서 갈라져
        // 정수리가 벌어져 보였다 (커튼이 아니라 가르마가 벗겨진 모양)
        // ⚠️ **가르마 끝을 «뾰족한 V» 로 두지 않는다** — 밑에서 접선이 마주 꺾여
        // 바늘처럼 보인다. 끝에서 접선을 «수평»으로 눕혀(102,67 → 100,67 → 98,67)
        // 둥근 U 로 만든다
        return `<path d="${BANG_OUT}
            C130,53 122,50 112,56 C106,60 102,67 100,67
            C98,67 94,60 88,56 C78,50 70,53 67,63.6 Z" fill="${c}"/>`;
      case 'sheer':       // 시스루뱅 — 성긴 잔머리 사이로 이마가 비친다.
        ...
        // ⚠️ **톱니(Q 로 판 삼각 이빨)를 쓰지 않는다** — 골이 바늘처럼 뾰족하다.
        // `S` 로 이어 붙이면 앞 구간의 접선이 그대로 이어져 **봉우리도 골도 둥근**
        // 물결이 된다 (수치를 흔들 때도 이 성질은 유지된다)
        return `<path d="${BANG_OUT}
            C131,54 128,55 125,60 S119,51 115,58 S106,51 102,58
            S93,51 89,58 S80,52 76,58 C73,60 70,60 67,63.6 Z" fill="${c}"/>`;
      case 'plain':       // 웨이브 (앞머리) — **처음에 있던 그 앞머리**.
        // 밑단이 봉우리 셋으로 물결진다. 예전 「기본(스트레이트 뱅)」이 이 모양이었는데
        // 인트로 공주의 매끈한 한 호로 바꾸면서 사라졌다 — 그 모양을 되살린 것이다.
        // ⚠️ 바깥 윤곽은 `BANG_OUT` 을 쓴다 (그것은 «머리의 꼭대기»라 다섯이 같다).
        // 안쪽 변만 옛 값을 이 윤곽의 끝(63.6)에 맞춰 3.6 내렸다
        // ⚠️ 옛 값(`C126,52 116,50 108,58 …`)은 골에서 접선이 꺾여 각이 졌다.
        // `S` 로 이어 붙여 봉우리도 골도 둥글게 편다 — 모양(봉우리 셋)은 그대로다
        return `<path d="${BANG_OUT}
            C129,54 124,51 117,56 S107,62 100,56 S93,51 83,56
            S72,60 67,63.6 Z" fill="${c}"/>`;
      default:            // 일자뱅 (스트레이트) — **인트로 공주와 같은 앞머리**
        ...
        // ⚠️ **한 번 더 옮겨 왔다.** 예전에는 「세로 38 → 30 · 가로 반폭 34 → 32」로
        // 어림해서 줄였는데, 그러면 바깥 윤곽이 공주보다 낮고 좁아 정수리 모양이 달랐다.
        // 지금 **바깥 호는 위의 얼굴 포개기 배율(33/34 · 35/33)을 그대로** 쓴다.
        //
        // ⚠️ **헤어라인(안쪽 호)만은 공주 값을 안 쓴다** (40.3 이 아니라 46.5).
        //
        // 아바타는 공주보다 **얼굴이 세로로 길고 눈이 낮게 붙어**(눈 중심 y75 ·
        // 얼굴 반높이 35) 같은 자리에 헤어라인을 두면 이마가 훨씬 넓어 보인다 —
        // 「이마가 넓다」고 신고받은 자리다. 배율로 옮긴 값이 아니라 **눈까지의
        // 거리로** 맞춘다 (헤어라인 46.5 → 눈 위 67 까지 20.5px).
        // ⚠️ 한 번에 50 까지 내렸다가 「너무 내려왔다」로 절반 되돌린 값이다 —
        // 43(옛값)과 50 의 가운데다. 여기를 또 흔들 일이 있으면 **표정 열둘을
        // 눈으로 보고** 정한다 (수치만으로는 이마의 «느낌»을 못 잰다)
        //
        // 아래로는 눈썹이 바닥이다. 화난 얼굴·뾰로통한 얼굴의 눈썹이 y58 언저리라
        // 여기서 더 내리면 앞머리가 눈썹을 덮는다.
        // 위로는 `checkavatar` 의 「정수리에 살색」이 43 을 바닥으로 잡는다 —
        // 커튼뱅의 가르마가 y40 에서 열려 **정수리가 갈라져 보이던** 버그를 잡는
        // 창이라 좁힐 수가 없다 (그 살색과 이마의 살색이 같은 높이에서 시작한다)
        // ⚠️ **인트로 공주의 앞머리를 그대로 옮긴 것이다.**
        // 공주의 `M116,174 C114,146 128,136 150,136 C172,136 186,146 184,174` 를
        // 위의 얼굴 포개기 배율(33/34 · 35/33)로 옮긴 값이고, 안쪽 호도 마찬가지다.
        //
        // ⚠️ **한 번 「타원의 호」로 바꿨다가 되돌렸다.** 뒤통수와 같은 타원(ry 만 늘린 것)
        // 으로 두면 이음매가 수학적으로 정확히 맞는데, 그러면 정수리가 공주보다
        // 둥글고 넓어져 **「예전(뒤통수 44.7) 으로 되돌아간 것 같다」**는 신고를 받았다.
        // 이 얼굴에서 맞는 것은 «정확한 이음매»가 아니라 «공주와 같은 선»이다.
        //
        // ⚠️ **헤어라인만은 43 이다** (공주 값 40.3 이 아니라). 40.3 에 두면
        // `checkavatar` 의 「정수리에 살색」이 잡는다 — 사이드뱅이 얼굴 꼭대기까지
        // 못 올라오던 것과 커튼뱅 가르마가 정수리에서 열리던 것을 **둘 다 실제로
        // 잡아내는** 검사라(사보타주로 확인했다) 창을 좁힐 수가 없다.
        // 아바타는 공주보다 얼굴이 세로로 길어 이마가 원래 더 넓으므로 3px 은 그 몫이다
        return `<path d="${BANG_OUT}
          C129.1,51 117.5,43 100,43 C82.5,43 70.9,51 67,63.6 Z" fill="${c}"/>`;
    }
  }
  ...
  //
  // ⚠️⚠️ **여유(`pad`)를 0 으로 두지 않는다 — 한때 셋 중 둘이 0 이었다.**
  // 목은 곧은 기둥이 아니라 `neckShape` 의 `mid`(=107) 부터 **바깥으로 벌어진다**.
  // 여유가 0 이면 옷깃이 그 자락을 **덮어 버려** 목이 넥라인 바닥까지 «같은 폭»으로
  // 이어진다 — 재 보면 맨몸의 목은 13px 인데 라운드넥을 입히면 **26px**이었다
  // (「목이 신체 비례에 비해 좀 길어진 듯 하고 드레스의 목파임 곡선이
  // 부자연스럽다」로 신고받았다 · 2026-09-24). 스퀘어 줄에 **이미 적혀 있던**
  // 「목에 붙여 파면 목이 그대로 이어져 굴뚝처럼 보인다」가 나머지 둘에서
  // 그대로 일어나고 있었다 — 규칙을 한 줄에만 적어 두면 이렇게 된다.
  //
  // ⚠️ **작은 여유(1~2)가 제일 나쁘다.** 옷깃이 자락을 «덮지도 비키지도» 못하고
  // 가로질러 잘라, 그 사이에 뾰족한 살 조각이 남는다 — 「옷의 목둘레와 신체
  // 목둘레가 안 맞는다」로 신고받았던 그 턱이다 (pad 1 → 1.0px). 여유는
  // **자락 바깥**까지 나가야 한다: 그러면 자락이 통째로 넥라인 «안»에 들어와
  // 목 → 쇄골 → 가슴으로 읽힌다. 지금 셋 다 5 이상이다 (`npm run neck` 으로 확인)
  const NECK_CUT = {
    // 라운드 — 맨몸의 목 길이(13px)와 같아지는 자리로 잡았다 (pad 0 은 16.5px 였다)
    round:  { pad: 6,  d: 9 },
    // 브이넥 — 여유가 0 이면 목이 V 의 «꼭짓점»까지 실오라기로 이어진다
    v:      { pad: 5,  d: 24 },
    // 스퀘어는 처음부터 이렇게 돼 있었다 — **넓고 얕다.**
  ...
  function headRatio(tune, w) {
    ...
    return lerpN(HEAD_K_SLIM, HEAD_K_FAT, b) * (1 + 0.06 * b) * kFace
      / (bodyScaleX(b) * bodyShrink(kFace));     // `build` 의 kx 와 같은 값 (한 곳에서 나온다)
  }
  ...
  //
  // ⚠️ **어깨 몫에 몸통 배율을 «또» 곱하지 않는다.** 그렇게 두었더니 바디파츠를
  // 다 같이 내렸을 때 `hk`(얼굴)와 `kTorso`가 **겹쳐 곱해져 제곱으로** 가늘어졌다 —
  // 최소에서 비례값의 **−26%**, 최대에서 **+26%** 였다 (「목 가운데 그림자가 가늘다」로
  // 신고받았다). 목은 **머리를 따라간다**: `hk` 하나만 곱하면 세 가지가 다 맞는다 —
  // ① 몸 전체를 줄이면 목도 «비례»해서 준다 ② 얼굴만 줄여도 목/머리 비가 그대로다
  // (「목과 머리」) ③ 100% 는 한 글자도 안 바뀐다.
  // 대신 **몸통만 줄인 몸에서는 목이 안 가늘어진다** — 목은 어깨가 아니라 머리가
  // 이고 있는 것이라 그쪽이 맞는다 (물어보고 정했다)
  function neckHalfOf(tune, w) {
    const hk = headRatio(tune, w) / HEAD_K_SLIM * NECK_SLIM;
    return hk * (33 * HEAD_K_SLIM * NECK_OF_FACE
          + (BODY.torsoR - 100) * NECK_OF_SHOULDER) / 2;
  }
  ...
  function build(outfit, body, tune) {
    ...
    // 몸무게를 tune 에 실어 내려보낸다 — 살은 부위마다 다르게 붙는다 (`FAT_W`)
    tune = withWeight(tune, w);
    ...
    //
    // ⚠️⚠️ **그 바닥(`NECK_MIN`)은 「목 길이」를 줄이는 쪽의 «축»이기도 하다.**
    // 통통한 몸은 여유가 거의 없다 — 검사가 못 박은 하한이 5px 인데 손대기 «전»에
    // 이미 5.2px 이었다(0.2px). 그래서 목을 통째로 곱해서 줄이면(0.7배) **반드시
    // 거기가 먼저 깨진다**(4.1px · 턱이 어깨에 얹힌다). 줄이는 것은 **바닥 «위»의
    // 여유뿐**이라야 날씬한 몸만 짧아지고 통통한 몸은 그대로 남는다
    // (10.4 → 8.7px · 5.2 → 5.1px).
    const NECK_SINK = 9, NECK_MIN = 7;
    // 바닥 위의 여유를 «얼마나 남길지».
    // ⚠️ **사람이 보고 고른 값이다** — 그려 놓고 골랐다. 올리면 목이 다시 길어지고,
    // 내리면 머리가 어깨에 얹힌다
    const NECK_KEEP = 0.7;
    ...
    const sink0 = Math.min(NECK_SINK * overOf(tune, 'face'),
      Math.max(0, neckLenAt(chin0) - NECK_MIN));
    // ⚠️⚠️ **목은 «머리를 따라» 앉는다 — 화면에 못 박힌 띠가 아니다** (2026-09-24 ·
    // 「바디파츠 50일 때와 100일 때 목이 좀 긴 것 같다」로 신고받았다).
    // 바디파츠를 «다 같이» 내리면 두 힘이 서로를 지운다: 머리가 줄면 턱이 이음점
    // (`NECK_Y`) 쪽으로 내려오는데, 그만큼 몸을 세로로 늘려(`bodyKy`) 머리를 도로
    // 올린다(`dy`). 그래서 **턱 y92.25 · 어깨 y105.5 가 50·60·75·100% 에서 «완전히
    // 같았고»**, 머리만 79 → 39px 로 반이 되어 **목÷머리가 0.165 → 0.338 로 두 배**가
    // 됐다 — 작은 머리에 긴 목이 붙은 꼴이다.
    // ⚠️ **위(`NECK_SINK`)는 «얼굴이 커질 때»만 보던 보정이다.** 아래쪽은 아무도 안
    // 보고 있었다 — 한쪽만 보는 잣대가 그 절반을 영영 못 보던 자리와 같은 종류다.
    // 지금은 **머리 높이에 견준 목 길이**를 한 곳에서 잡는다.
    // ⚠️ **늘리지는 않는다**(`max(0, …)`). 머리가 큰 쪽은 「머리가 클수록 목이 짧은
    // 편이 귀엽다」로 사람이 골라 둔 값이라, 여기서 되돌리면 그 결정이 뒤집힌다 —
    // 얼굴 125·150% 는 한 픽셀도 안 바뀐다
    // ⚠️⚠️ **«상한»이 아니라 «비율»로 줄인다.** 절대값 상한으로 두었더니 모두가
    // 같은 천장에 붙어 **차이가 통째로 뭉개졌다** — 「날씬할수록 목이 길다」
    // (`NECK_FAT`)와 「몸통을 가늘게 하면 목이 드러난다」(`NECK_LIFT_TORSO`)가
    // 둘 다 사라져 검사가 잡았다 (체형 0 과 0.25 의 목이 같아지고, 몸통 50% 와
    // 150% 의 목이 같아졌다). 곱셈으로 줄이면 그 차이가 «비율 그대로» 남는다.
    // ⚠️ 머리 몫은 **1 을 넘지 않는다** — 큰 머리 쪽은 `NECK_SINK` 가 이미 맡았고,
    // 여기서 늘리면 「머리가 커질수록 목이 짧다」가 뒤집힌다
    // ⚠️⚠️ **머리 몫(`hs`)은 «여유»가 아니라 «목 전체»에 곱한다.** 여유에만 곱하면
    // 바디파츠를 내려도 바닥(`NECK_MIN`)이 그대로 남아, 반으로 줄인 머리에 100% 짜리
    // 목 바닥이 붙는다 — 고치려던 바로 그 그림이다. 목이 통째로 머리를 따라가야
    // 목÷머리가 어느 단계에서나 같다. ⚠️ **바닥도 같이 줄지만 그것이 맞는다** —
    // 「5px」은 «기본 머리»에서 턱이 어깨에 얹히지 않는 몫이라, 머리가 반이면
    // 그 몫도 반이다. 통통·몸통 축은 바디파츠 100%(hs=1)에서 도는 자리라 한 글자도
    // 안 바뀐다 (그래서 위의 바닥 규칙이 거기서 그대로 산다)
    const hs = Math.min(1, headK * kFace);
    const now0 = neckLenAt(chin0 + sink0);
    const want = (NECK_MIN + Math.max(0, now0 - NECK_MIN) * NECK_KEEP) * hs;
    const headDrop = dropBody + sink0 + Math.max(0, now0 - want);
    ...
    const kx = bodyScaleX(w) * kBody, hx = headK * (1 + 0.06 * w) * kFace;
