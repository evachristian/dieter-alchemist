...
(function () {
  ...
  function neckGusset(color, neck, neckKind) {
    ...
    const f = n => (+n).toFixed(2);
    // ── 턱 밑은 **살**이 덮는다 — 옷은 «제 어깨선»에서 시작한다 (목깃이 없다).
    //
    ...
    // 덮는 자리는 그대로고 «보이는 옷의 위 끝»만 어깨선으로 내려온다.
    // ⚠️ **옆변을 목 쪽으로 오므리지 않는다.** 오므리면 옷이 목을 «동글동글» 파고
    // 들어가 라운드넥 스쿱이 된다 (그렇게 만들었다가 「목이 동글동글 파여 있다」로
    // 신고받았다). 살받침은 **옷깃 받침과 «같은 옆변»**을 쓰고 어깨선에서 자른다 —
    // 그러면 옷의 위 끝이 어깨선 한 줄로 곧게 서고 목 옆에는 옷이 없다
    ...
    let skinCap = '';
    if (color !== SKIN && lowNeck && CLOTH_TOP_Y > cut + 1) {
      // 옆변은 받침의 2차 곡선 «그 자체»다 — 어깨선에서 잘라 쓴다.
      // ⚠️ 곡선을 새로 그리지 않고 **같은 곡선을 나눠 쓴다**: 두 벌로 두면 한쪽만
      // 고쳤을 때 살과 옷 사이에 실오라기가 생긴다 (이 파일이 여러 번 겪은 일이다).
      //   y(t) = sy + (cut − sy)·(1−t)²  →  어깨선에 닿는 t 를 바로 푼다
      const u = Math.sqrt(Math.max(0, Math.min(1, (CLOTH_TOP_Y - sy) / (cut - sy))));
      const t = 1 - u;
      const bez = (a, b, c) => {               // 드 카스텔조 — t 까지의 조각
        const q1 = a + (b - a) * t;
        return [q1, q1 + ((b + (c - b) * t) - q1) * t];
      };
      const [c1x, c2x] = bez(xc, cx, sx);
      const [c1y, c2y] = bez(cut, sy, sy);
      skinCap = `<path fill="${SKIN}" d="M${f(100 - xc)},${f(cut)}`
        + ` A${f(rx)},${f(ry)} 0 0 0 ${f(100 + xc)},${f(cut)}`
        + ` Q${f(100 + c1x)},${f(c1y)} ${f(100 + c2x)},${f(c2y)}`
        + ` H${f(100 - c2x)} Q${f(100 - c1x)},${f(c1y)} ${f(100 - xc)},${f(cut)} Z"/>`;
    }
