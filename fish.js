...
(function () {
  ...
  //
  // ⚠️ **이 둘의 «비»가 곧 난이도다** — 눈금이 안 줄어들려면 씨름하는 동안 그물 안에
  // `CATCH_MS / (CATCH_MS + DRAIN_MS)` 만큼은 붙들고 있어야 하고, 그 아래로 내려가면
  // 눈금이 0 으로 떨어져 놓친다. 6400 일 때 그 손익분기가 **38.5%** 였는데,
  // 사람 셋을 붙여 재 보니 보통(반응 280ms) 36% · 느긋한(420ms) 35% 라
  // **평균이 분기점 «아래»였다** — 「낚시 너무 어렵다」가 정확히 이 수치다.
  // 9200 이면 분기가 **30.3%** 라 보통 사람이 여유를 두고 넘는다
  const DRAIN_MS = 9200;
  ...
  // 그물 높이 (건질수록 좁아진다). ⚠️ **좁히는 몫도 같이 줄인다** — 0.24/0.012 로 두면
  // 아홉 번째에 0.15 라, 상한(18개)에 닿으려는 사람에게만 유독 어려워진다
  const BAR_H0 = 0.30, BAR_SHRINK = 0.008, BAR_MIN = 0.22;

  // ─── 물고기의 움직임 ───
  // 한 자리를 정해 그리로 가다가, 이따금 마음을 바꾼다. **속도는 건질수록 조금 빨라진다** —
  // ⚠️ 많이 올리지 않는다: 2분짜리에서 끝에 가서 못 따라가면 그건 벌이다
  const FSH_SPD0 = 0.00034, FSH_SPD_UP = 0.000014, FSH_SPD_MAX = 0.00055;
  // ⚠️ **마음을 바꾸는 «간격»이 속도보다 어렵게 만든다.** 420ms 마다 자리를 새로 뽑으면
  // 사람이 따라붙는 데 드는 시간(반응 + 그물을 미는 시간)보다 짧아서, 쫓아가는
  // 도중에 이미 다른 데로 간다 — 아무리 잘해도 못 따라잡는 구간이 생긴다
  const FSH_T_MIN = 620, FSH_T_MAX = 1500;
