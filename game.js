...
//
// ⚠️ **다섯이 다 같은 종류의 문이 아니다.** 붙일 자리를 고르기 전에 이것부터 갈라야 한다:
//
//  · **이야기가 여는 것** (부엌 · 운동) → 퀘스트에 건다
//  · **뱃지가 달린 기록** (흡입 · 수확) → 그 **뱃지를 켜는 «내용물»** 에 건다
//  · **점수 문** (밭) → 여신(리그와 같은 문 · `farmOpen`)
//
// ⚠️⚠️ 가운데가 핵심이다 — 흡입·수확은 **붉은 뱃지가 달린 버튼**이라, 뱃지를 켜는
// 조건과 버튼을 켜는 조건이 갈리면 반드시 둘 중 하나가 난다:
//  · 버튼이 **먼저** 열리면 → 누를 것이 없는 빈 기록 화면
//  · 버튼이 **나중에** 열리면 → **뱃지가 갈 곳을 잃는다.** 혼자 먹은 밤이 셋 쌓였는데
//    알릴 자리가 없고, 크리처가 사흘치를 가져다 놨는데 아무도 안 알려 준다 —
//    화면에는 오류 하나 안 뜬다
//
// 그 둘의 내용물이 **마침 퀘스트의 목표**라(`q_egg` = 크리처 한 마리), 퀘스트에 안
// 매달아도 결과는 「퀘스트 사이사이」 그대로 놓인다.
function actOpen(id) {
  if (!S.tutorialDone) return false;                 // 튜토리얼 중에는 전부 숨는다
  if ((S.roomActs || []).includes(id)) return true;  // 개발용으로 켠 것 · 옛 세이브
  switch (id) {
    case 'kitchen':  return chefHired();              // 셰프가 왔는가
    case 'exercise': return questCleared('q_kitchen'); // 부엌을 세 번 다녀왔는가
...
function questCleared(id) { return (questState().done || []).includes(id); }

// ─── 🏃 운동 — `q_kitchen`(부엌 3번)을 끝내면 ───────────────────
//
// **부엌(채우기)과 운동(쓰기)은 한 쌍이다.** `EXERCISE.md` 의 축이 포만감·스태미나인데
// 포만감을 채우는 법을 배우는 퀘스트가 바로 `q_kitchen` 이다 — 세 번 다녀와서 「밥」을
// 익힌 다음 줄에 몸을 쓰는 쪽이 열린다.
//
// ⚠️ **순서를 뒤집으면 벌이 먼저 온다.** 운동은 포만감을 더 빨리 깎으므로, 부엌을
// 모르는 사람에게 먼저 주면 **혼자 먹은 밤이 잦아지는데 고칠 방법은 안 배운 상태**가
// 된다 — 「덜 먹는 게임이 아니라 혼자 먹지 않는 게임이다」(EXERCISE.md)와 어긋난다.
// 하루 한 번짜리 목표라 실제로 사흘이 걸린다 — 졸업 직후에 몰리지 않는다.
//
// ⚠️ 다른 후보였던 `q_sip`(물약 5번 마시기 · at 32)은 뜻은 좋은데 **너무 늦다** —
// 그 무렵이면 이미 혼자 먹은 밤이 여럿 쌓여 있다 (물어보고 `q_kitchen` 으로 정했다)

// ─── 🍗 흡입 — 첫 혼자 먹은 밤 (퀘스트가 아니다) ────────────────
//
// **얻는 것이 아니라 «생긴 것»이다** — 퀘스트 보상으로 벌의 기록을 주는 것은 말이 안 된다.
// ⚠️ **숨기면 통보를 잃는다.** 밤은 부엌이 열린 첫날부터 생길 수 있는데 버튼이 퀘스트
// 뒤에 있으면 그동안의 밤이 **아무 데도 안 보인다** — 「부엌을 선택 콘텐츠 뒤에 숨기지
// 않는다」의 짝이다: 피할 길만 열어 두고 **결과를 볼 창구를 잠그면** 같은 사고가
// 반대쪽에서 난다. 안 거른 사람에게는 영영 안 뜨고, 그것이 이 버튼의 뜻이다.
//
// ⚠️⚠️ **`bingeCount()`(아직 «안 본» 밤)로 판정하지 않는다.** 그건 장면을 다 보면
// 0 이 되는 큐라, 컷씬을 끝까지 넘기는 순간 **버튼이 눈앞에서 사라진다** — 게다가
// 0 일 때 누르면 「볼 것이 없다」(`bs_none`)고 말해 주게 돼 있으니, 그 갈래가 영영
// 안 돌게 된다. 「한 번이라도 있었는가」는 **되돌아가지 않는 값**이어야 한다.
function everBinged() { return ((S.record || {}).aloneNights || 0) > 0; }

// ─── 🌾 밭 — 여신(매력 100) · `farmOpen()` 그대로 ────────────────
//
// 밭은 **리그와 같은 문**이다(`D.LEAGUE.openAt`). 탐험의 밭 탭이 이미 이 문을 쓰고,
// 마이 룸 🌾 는 여신이면 **그 탭으로 보내는 버튼**이다 — 같은 것을 두 자리에서 안
// 보여 주기로 한 규칙이다. ⚠️ 여기에 퀘스트 조건을 «또» 걸면 **한 것을 두 번 잠그는**
// 것이 된다 (마을 해금에서 이미 정한 규칙). 그래서 `actOpen('farm')` 이 `farmOpen()` 을
// **그대로** 부른다 — 「버튼은 보이는데 눌러도 안 열린다」 갈래가 새 플레이어에게는 없다
// (옛 세이브는 `roomActs` 로 먼저 통과하므로 지금까지 보던 「아직 못 연다」 시트가 그대로다)
...
function chefHired() {
  return (S.seenCuts || []).includes('c_meet_in') || questCleared('q_meet');
}
...
function renderQuestChip() {
  ...
  const full = now >= max;
  // ⚠️⚠️ **맥박은 「인트로 컷씬을 아직 안 봤다」는 뜻이다.** 예전에는 「한 걸음도 안 뗐다」
  // 였는데, 그러면 **누를 이유가 없는 동안에도 계속 뛴다** — 상태형 목표(마을·호감도)는
  // 한 걸음 떼는 데만 며칠이 걸려서 그동안 내내 재촉하는 꼴이었다.
  // 움직이는 것은 «누르면 지금 무슨 일이 일어나는» 자리에만 준다는 규칙 그대로다:
  // 안 본 인트로가 있으면 누르는 순간 **새 장면이 돌고**(`openQuest` 의 `playCut`),
  // 한 번 본 뒤로는 눌러 봐야 시트가 열릴 뿐이다.
  // ⚠️ **인트로 칸부터 본다** — 없는 퀘스트를 `includes(undefined)` 로 재면
  // «영영 안 본 것»이 되어 그 칩만 끝까지 뛴다 (지금 열일곱은 다 갖고 있다)
  const inCut = q.cut && q.cut.in;
  const fresh = !!inCut && !(S.seenCuts || []).includes(inCut);
...
// 「🧺 수확」 버튼이 열리는 조건 — **가진 크리처 중에 생산하는 아이가 있는가.**
// 이것이 곧 `q_egg`(크리처 한 마리)의 목표라, 조건은 상태인데 자리는 퀘스트 사이다.
//
// ⚠️ **`producers()`(애착·동행에 «세운» 아이)로 판정하지 않는다.** 크리처를 얻어도
// 세우기 전에는 거짓이라, 버튼이 `q_egg` 가 아니라 **옷장을 열어 본 순간**에 뜬다 —
// 그러면 「퀘스트 사이사이」가 아니라 아무 때나 나타나는 버튼이 된다.
// 세운 아이가 없는 동안은 기록 시트가 `pd_none_pet` 으로 그렇다고 말해 준다.
//
// ⚠️ 반대로 「`q_egg` 를 «끝내면»」으로도 두지 않는다 — 크리처를 얻는 순간 생산이
// 이미 시작되므로(`settleProduce`), 보상을 받으러 가기 전에 **뱃지만 켜질 수 있는
// 날**이 생긴다. 뱃지와 버튼은 같은 것을 봐야 한다
function hasProducer() {
  return (S.creatures || []).some(id => {
    const c = creatureOf(id);
    return !!(c && c.makes && D.INGREDIENTS[c.makes.id]);
  });
}
...
//
// **늘 혼자였던 것을 요리사 클레멘이 갈랐다.** 부엌에 갔던 밤은 폭식이 아니라
// **함께한 식사**가 된다 (포만감은 차고, 깎이는 것은 없다) — 아래 `warm` 이 그것이다.
...
// (그려지는 폭 328 × 260/366 ≈ 233px, .char-aura 240px 안에 들어온다.
//  세로 상자가 348 → 366 으로 열리면서 높이도 312 → 328 로 같이 올라 **몫이 그대로**다)
//
...
function princessFigure() {
  ...
  // 여기만 348 로 굳어 있으면 아바타 상자가 366 으로 열린 순간 **공주만 커진다.**
...
function renderPalRow() {
  // ⚠️ **동행·시간대가 필드 갈래에서만 뜨는 것은 «구조»가 지킨다** — 이 줄은
  // `#gatherPanel-field` 안에 살아서 갈래가 바뀌면 판째로 사라진다 (index.html).
  // 한때 탭 «위»에 두고 여기서 `line.hidden = gatherTab !== 'field'` 로 접었는데,
  // 그건 **JS 가 지키는 약속**이라 부르는 것을 한 번 빠뜨리면 조용히 어긋났다.
  // 자리를 옮기며 그 줄을 지웠다 — 안 쓰는 규칙을 남겨 두면 다음 사람이 그것을
  // 「아직 필요한 것」으로 읽는다 (`.pal-line[hidden]` 짝 CSS 를 지운 것과 같은 자리다)
...
//   얼굴  예전에는 120% 에서 머리 위가 잘려 114 였다. `VB` 를 위로 18 열고
//         (348 → 366) **115% 를 넘으면 몸을 줄여**(`bodyShrink`) 목이 내려오게 해서
//         **150 까지** 올렸다. 더 올리려면 상자를 또 열어야 하고,
...
//   허벅지·종아리는 조합에서도 200% 까지 «잘리지는» 않았지만 150 으로 낮췄다 —
//         잘리느냐가 아니라 **그 몸이 쓸 만하냐**로 정한 값이다. 200% 에서
//         허벅지 반폭이 70.5px(기본의 1.9배)라 다리 둘이 서로 파고들어
//         사람 몸으로 안 읽혔다. 다른 부위와 같은 150 으로 맞춘다
// **한 파츠씩만 재면 안 된다** — 조합에서 처음 넘쳤다. 바꾸려면 조합으로 다시 재라.
// ⚠️ **상한을 낮추면 이미 저장된 값이 그 위에 있을 수 있다.** `bodyTune` 이
// 읽을 때 범위 안으로 당긴다 — 여기만 고치고 그것을 빼면 슬라이더가 상한 밖에서 시작한다.
