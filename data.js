...
const FIELD_TYPES = [
  ...
  { k: 'driller', mini: 'driller', name: 'ft_driller', tag: 'mini_tag_dr' },  // 소풍 바위 — 바위 부수기
];
// <<<GEN:maptype
const MAP_TYPES = {
  // 포근 평야 지대
...
const SPEAKERS = [
  ...
  { id: 'sp_althea', name: '알테이아', hair: 'updo', hairColor: '#eeeaf2', beard: 'none',
    ...
    // ⚠️ **이름 하나에 그림 하나다.** 예전에는 손그림 아홉에 서른여덟 이름이 겹쳐 붙어
    // 있어서, 「지침」도 「부탁」도 다 같은 얼굴이었다. 지금은 손그림 아홉 뒤에
    // **부품 표**(`intro.js` 의 `FZ_FACE`)가 걸려 있어 나머지도 제 얼굴을 갖는다
    moods: { def:   { art: 'idle' },
             flat:  { art: 'flat' },
             think: { art: 'think' },
             sad:   { art: 'sad' },
             ...
             scold: { art: 'glance' },
             doubt: { art: 'doubt' },
             // 대노 — 「혼자 먹은 밤」을 다섯 번 보고 나면 나온다 (scold 는 곁눈질이라 약하다)
             cross: { art: 'cross' },
             angry: { art: 'cross' },
             cold:  { art: 'cold' },
             warm:  { art: 'warm' },
  ...
  { id: 'sp_gwiriel', name: '그위리엘', hair: 'long', hairColor: '#7b5640', beard: 'none',
    ...
    // ⚠️ **이름 하나에 그림 하나다.** 예전에는 서른여덟 이름이 그림 열셋에 겹쳐 붙어
    // 있어서, 대사가 「지침」이라고 적어도 「부탁」과 같은 얼굴이 나왔다.
    // 지금은 손으로 그린 열셋 뒤에 **부품 표**(`intro.js` 의 `PZ_FACE`)가 걸려 있어
    // 나머지 스물다섯도 제 얼굴을 갖는다. 여기 `art` 는 그 이름을 그대로 넘긴다
...
const WARDROBE = {
  ...
  hair: [
    ...
    { id: 'hair_long_plain', slot: 'hair', kind: 'long', name: '긴 생머리 웨이브', back: 'long', bang: 'plain', color: '#7b5640' },
    ...
    { id: 'hair_bob_plain', slot: 'hair', kind: 'bob', name: '단발 웨이브', back: 'bob', bang: 'plain', color: '#7b5640' },
    ...
    { id: 'hair_twin_plain', slot: 'hair', kind: 'twin', name: '양갈래 웨이브', back: 'twin', bang: 'plain', color: '#7b5640' },
    ...
    { id: 'hair_ponytail_plain', slot: 'hair', kind: 'ponytail', name: '포니테일 웨이브', back: 'ponytail', bang: 'plain', color: '#7b5640' },
    ...
    { id: 'hair_wave_plain', slot: 'hair', kind: 'wave', name: '웨이브', back: 'wave', bang: 'plain', color: '#7b5640' },
    ...
    { id: 'hair_bun_plain', slot: 'hair', kind: 'bun', name: '올림머리 웨이브', back: 'bun', bang: 'plain', color: '#7b5640' },
  ],
  // circlet — 4 × 5 = 20
  circlet: [
  ...
  shoes: [
    ...
    { id: 'shoes_glass', slot: 'shoes', kind: 'flat', name: '유리구두', rise: 0, heel: 5, finish: 'gloss', color: '#b8e4ff', emoji: '👠' },
...
// GEN:wardrobe>>>
  ...
  expression: [
    ...
    { id: 'exp_warm',      slot: 'expression', kind: 'warm',      name: '다정', emoji: '😊' },
    { id: 'exp_proud',     slot: 'expression', kind: 'proud',     name: '우쭐', emoji: '😏' },
    { id: 'exp_smirk',     slot: 'expression', kind: 'smirk',     name: '능글', emoji: '😼' },
    { id: 'exp_flat',      slot: 'expression', kind: 'flat',      name: '무표정', emoji: '😐' },
    { id: 'exp_meh',       slot: 'expression', kind: 'meh',       name: '심드렁', emoji: '🙄' },
    { id: 'exp_sleepy',    slot: 'expression', kind: 'sleepy',    name: '졸림', emoji: '😪' },
    { id: 'exp_think',     slot: 'expression', kind: 'think',     name: '생각 중', emoji: '🤔' },
    { id: 'exp_doubt',     slot: 'expression', kind: 'doubt',     name: '미심쩍음', emoji: '🧐' },
    { id: 'exp_worry',     slot: 'expression', kind: 'worry',     name: '걱정', emoji: '😟' },
    { id: 'exp_sad',       slot: 'expression', kind: 'sad',       name: '슬픔', emoji: '🙁' },
    { id: 'exp_cry',       slot: 'expression', kind: 'cry',       name: '울음', emoji: '😢' },
    { id: 'exp_angry',     slot: 'expression', kind: 'angry',     name: '화남', emoji: '😠' },
    { id: 'exp_grit',      slot: 'expression', kind: 'grit',      name: '참음', emoji: '😤' },
    { id: 'exp_shy',       slot: 'expression', kind: 'shy',       name: '수줍음', emoji: '☺️' },
    { id: 'exp_ohh',       slot: 'expression', kind: 'ohh',       name: '아하', emoji: '😮' },
    { id: 'exp_star',      slot: 'expression', kind: 'star',      name: '기대', emoji: '🤩' },
    { id: 'exp_love',      slot: 'expression', kind: 'love',      name: '반함', emoji: '😍' },
    { id: 'exp_faint',     slot: 'expression', kind: 'faint',     name: '기절', emoji: '😵' },
    { id: 'exp_haha',      slot: 'expression', kind: 'haha',      name: '박장대소', emoji: '😆' },
    { id: 'exp_tease',     slot: 'expression', kind: 'tease',     name: '메롱', emoji: '😝' },
    { id: 'exp_pout',      slot: 'expression', kind: 'pout',      name: '토라짐', emoji: '😒' },
    { id: 'exp_glare',     slot: 'expression', kind: 'glare',     name: '노려봄', emoji: '😾' },
    { id: 'exp_suspect',   slot: 'expression', kind: 'suspect',   name: '흘김', emoji: '😑' },
    { id: 'exp_dizzy',     slot: 'expression', kind: 'dizzy',     name: '어질어질', emoji: '😵‍💫' },
    { id: 'exp_weary',     slot: 'expression', kind: 'weary',     name: '지침', emoji: '😮‍💨' },
    { id: 'exp_relief',    slot: 'expression', kind: 'relief',    name: '안도', emoji: '😌' },
...
const HAIR_AXES = {
  ...
  bang: [
    ...
    { id: 'hairbang_plain', k: 'plain', name: '웨이브' },
  ],
};
// GEN:hairaxes>>>
...
const ENERGY = {
  ...
  // ─── 미니게임 맵의 AP ───────────────────────────────────────
  //
  // **미니게임은 «한 판이 돌려주는 개수»에 비례해서 받는다:**
  //
  //     미니게임 AP = 지대 AP × 한 판 상한 × miniApK
  //
  // ⚠️ 예전에는 미니게임 맵도 **평범한 채집과 같은 AP**(평야 6)였다. 그런데 한 판이
  // 6~20개를 돌려주니 **AP당 6~20배**였고, 꾹 누르기가 420ms 에 하나(`HOLD_EVERY`)라
  // 하루치 1000 AP 는 70초면 다 쓴다 — 즉 **하루 AP 를 다 쓰고 나서 미니게임을 따로 더
  // 돌 수 있었다.** 값을 AP 로 매겨 놓고 실제로는 «시간»으로만 받고 있던 셈이다.
  //
  // ⚠️ 그렇다고 AP당 효율을 평범한 채집과 **같게** 맞추면 안 된다 — 호두 20개가
  // 꾹 누르기로는 8.4초인데 미니게임은 2분이다. 같은 값이면 아무도 안 한다.
  //
  // 식이 깔끔하게 떨어진다 — 지대도 상한도 약분돼서 **배수 = 1/k · 본전 지점 = k** 다.
  // 지금 0.2 이니 **상한을 다 먹으면 5배 · 상한의 20% 만 먹어도 본전**이다.
  // 봇으로 잰 제일 못하는 손이 상한의 45% 쯤이라 **못한 판도 손해가 아니다**
  // (「틀려도 잃는 것이 없다」의 연장이다).
  //
  // ⚠️ **한 판 상한은 여기 안 적는다** — 그 게임의 모듈(`REWARD_MAX`)이 유일한 원본이고
  // game.js 의 `miniRewardMax()` 가 거기서 읽는다. 사본을 두면 게임을 고쳤을 때
  // 값만 옛 것으로 남는다. `checkbalance` 가 이 관계를 전부 지킨다
  miniApK: 0.2,
  // 조합에 **성공**하면 주는 현자의 결정.
  //
